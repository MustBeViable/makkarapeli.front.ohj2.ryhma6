...
async function airport_selection_function() {
  try {
    ...
    const location = await getData(
        `http://127.0.0.1:5000/player_location/${ide}`);
    airportMarkers.clearLayers();

    const locmarker = await L.marker([location.lattitude, location.longitude]).
        addTo(map);
    ...
    for (let i = 0; i <= 19; i++) {
      ...
      button.innerText = `Airport: ${result[`${i +
      1}`].name} Price: ${result[`${i + 1}`].price}`;
      ...
      button.addEventListener('click', async () => {
        ...
        await gamesectionstatusupdate('garbage', '0');
        await gamesectionstatusupdate('taxfree', '0');
